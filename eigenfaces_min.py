""" Implementation of facial recognition using eigenfaces (Algorithm 2).
...
def recognize_face(eigen_num: int, subj_img: tuple,
                   proj_err_limit=2850.00, coeff_err_limit=6.0):
    """Uses the eigenfaces method to recognize an image.

    This function uses the eigenfaces method, along with the second
    ...
    if projection_error > proj_err_limit:
        ...
        if face_error > coeff_err_limit:
            ...
            # the matched face.
            match = np.argmin(difference)
            match = match // imgs_per_subject
            if (match + 1) == subject:
                print("Got it right.")
            else:
                print("Got it wrong.")
